...
def benchmark_performance():
    ...
    qweight = torch.zeros_like(weight, dtype=torch.float8_e4m3fn, device="cuda")
    scales = torch.zeros((n_tiles_n, n_tiles_k), dtype=torch.float32, device="cuda")
...
def check_accuracy():
    ...
    triton_qweight = torch.zeros_like(weight, dtype=torch.float8_e4m3fn, device=device)
    triton_scales = torch.zeros((n_tiles_n, n_tiles_k), dtype=torch.float32, device=device)
    ...
    quantize_block_kernel[grid](
        weight, triton_qweight, triton_scales, n_tiles_n, n_tiles_k, n, k, block_n, block_k, FP8_MIN, FP8_MAX
    )
    
    # Compare results
    qweight_diff = torch.abs(pytorch_qweight.to(torch.float32) - triton_qweight.to(torch.float32))
    scale_diff = torch.abs(pytorch_scale - triton_scales.squeeze())
    ...
    print(f"Triton scale range: [{triton_scales.squeeze().min():.6f}, {triton_scales.squeeze().max():.6f}]")
    
    if torch.allclose(
        pytorch_qweight.to(torch.float32), triton_qweight.to(torch.float32), rtol=1e-3, atol=1e-5
    ) and torch.allclose(pytorch_scale, triton_scales.squeeze(), rtol=1e-3, atol=1e-5):
